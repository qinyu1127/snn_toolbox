...
def get_dataset(config):
    ...
    # _______________________________ aedat __________________________________#
        ...
        data_gen_args_mask = dict(rotation_range=0.2,
                                  width_shift_range=0.05,
                                  height_shift_range=0.05,
                                  shear_range=0.05,
                                  zoom_range=0.05,
                                  ...
                                  fill_mode='nearest',
                                  rescale=1 / 255,
                                  preprocessing_function=mask_process)

        # myGene = trainGenerator(30, 'data/membrane/train', 'image', 'label', data_gen_args, save_to_dir=None)
        img_val = imgGenerator(30, dataset_path, 'image', 'label', data_gen_args, save_to_dir=None)
        mask_val = maskGenerator(30, dataset_path, 'image', 'label', data_gen_args_mask, save_to_dir=None)
        testset = {'x_test': img_val, 'y_test':mask_val}
...
def maskGenerator(batch_size,train_path,image_folder,mask_folder,aug_dict,image_color_mode = "grayscale",
    ...
    mask_generator = mask_datagen.flow_from_directory(
        ...
        seed = seed)
    y_b_l = mask_generator.next()
    return y_b_l

def imgGenerator(batch_size,train_path,image_folder,mask_folder,aug_dict,image_color_mode = "grayscale",
                    mask_color_mode = "grayscale",image_save_prefix  = "image",mask_save_prefix  = "mask",
                    flag_multi_class = False,num_class = 2,save_to_dir = None,target_size = (256,256),seed = 1):
    '''
    can generate image and mask at the same time
